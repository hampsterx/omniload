...
:endpoint_url:
  The HTTP or HTTPS endpoint URL to connect to. Use only the scheme, host and
  optional port; paths, queries and fragments are not supported.
...
:region:
  AWS signing region for source reads (optional). Use the region required by
  your R2 endpoint, commonly `auto`.
