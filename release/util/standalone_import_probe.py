...
def read_requirement_set(pyproject: pathlib.Path) -> list[str]:
    ...
    def parse(value: object, where: str) -> "Requirement":
        ...
        if parsed.url is None and parsed.name.lower().endswith(ARCHIVE_SUFFIXES):
            raise Failure(
                f"{where} names an archive rather than a distribution: "
                f"{value!r}. uv would install the file of that name from the "
                "working directory.",
                status=2,
            )
        return parsed
...
def child_main(mode: str, stage: str, evidence_fd: int) -> int:
    ...
    try:
        if mode == "entry":
            return _child_entry(evidence_fd)
...
def _child_entry(evidence_fd: int) -> int:
...
def _report(mode: str, status: int, output: str, evidence: bytes, token: bytes) -> None:
    ...
    if status == 0 and token in evidence:
        return
